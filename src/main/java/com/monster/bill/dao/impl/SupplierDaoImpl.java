...
public class SupplierDaoImpl implements SupplierDao {
	...
	public static final String GET_SUPPLIERS = " select new com.monster.bill.models.Supplier(s.id, s.name, s.vendorNumber, s.vendorType, s.isActive, sub.name, sc.name, sc.contactNumber, s.approvalStatus) "
			+ " from Supplier s "
	...
	public List<Supplier> findAll(String whereClause, PaginationRequest paginationRequest) {
		...
		// group by clause
		finalSql.append(" group by s.id, s.name, s.vendorNumber, s.vendorType, s.isActive, sub.name, sc.name, sc.contactNumber ");
