...
import com.monster.bill.dao.QuotationDao;
...
import com.monster.bill.repository.QuotationGeneralInfoRepository;
import com.monster.bill.repository.QuotationHistoryRepository;
import com.monster.bill.repository.QuotationItemRepository;
import com.monster.bill.repository.QuotationItemVendorRepository;
import com.monster.bill.repository.QuotationPrRepository;
import com.monster.bill.repository.QuotationRepository;
import com.monster.bill.repository.QuotationVendorRepository;
import com.monster.bill.repository.SupplierContactRepository;
...
//		}
		...
		if (quotation.getId() == null) {
			...
			String documentSequenceNumber = this.documentSequenceService.getDocumentSequenceNumbers(transactionalDate, quotation.getSubsidiaryId(), FormNames.RFQ.getFormName(), false);
...
//				}
				...
				if (isSubmitted) {
					...
					List<PrItem> prItems = this.prItemRepository.findByPrIdAndItemId(quotationItem.getPrId(), itemId);
					if (CollectionUtils.isNotEmpty(prItems)) {
						for (PrItem prItem : prItems) {
							prItem.setRfqId(rfqId);
...
//						log.info("Quotation Item Vendor Inserted and History is updated.");
						...
						if (!quotationVendor.isPresent()) {
							...
							Optional<SupplierContact> supplierContact = Optional.empty();
							supplierContact = this.supplierContactRepository.findBySupplierIdAndIsPrimaryContactAndIsDeleted(vendor.getVendorId(), true, false);
							if (supplierContact.isPresent()) {
								quotationVendor.get().setContactName(supplierContact.get().getName());
								quotationVendor.get().setEmail(supplierContact.get().getEmail());
							}
...
//						log.info("Quotation Item Vendor Inserted and History is updated.");
		...
		if (isSubmitted) {
			for (QuotationPr quotationPr : quotation.getQuotationPrs()) {
				...
				Long unusedItemCount = prItemRepository.findUnprocessedItemsCountByRfq(prId);
...
//	}
	...
	private Quotation prepareRfq(RfqPoRequest rfqRequest, String prIdStr) {
		try {
			...
			String rfqNumber = this.documentSequenceService.getDocumentSequenceNumbers(transactionalDate , rfqRequest.getSubsidiaryId(), FormNames.RFQ.getFormName(), false);
				...
				if (purchaseRequisition != null) {
					...
					if (CollectionUtils.isNotEmpty(prItems)) {
						for (PrItem prItem : prItems) {
							...
							quotationItem.setRfqNumber(rfqNumber);
							quotationItem.setItemId(prItem.getId());
							...
							quotationItem.setPrNumber(prIds[i]);
							quotationItem.setPrId(prId);
							//quotationItem.setPrLocation(String.valueOf(purchaseRequisition.getLocationId()));
							quotationItem.setCreatedBy(username);
							quotationItem.setLastModifiedBy(username);
