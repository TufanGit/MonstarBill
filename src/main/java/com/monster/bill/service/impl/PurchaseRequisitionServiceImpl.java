...
import com.monster.bill.repository.LocationRepository;
import com.monster.bill.repository.PrItemRepository;
import com.monster.bill.repository.ProjectRepository;
import com.monster.bill.repository.PurchaseRequisitionHistoryRepository;
...
public class PurchaseRequisitionServiceImpl implements PurchaseRequisitionService {
	...
	public PurchaseRequisition save(PurchaseRequisition purchaseRequisition) {
		...
		if (purchaseRequisition.getId() == null) {
			...
			String documentSequenceNumber = this.documentSequenceService.getDocumentSequenceNumbers(transactionalDate, purchaseRequisition.getSubsidiaryId(), FormNames.PR.getFormName(), false);
...
//	}
	...
	public List<PrItem> findUnprocessedItemsByPrId(Long prId) {
		...
		items = this.purchaseRequisitionDao.findUnprocessedItemsByPrId(prId);
		return items;
	}
	...
	public List<IdNameResponse> findPendingPrForPo(Long subsidiaryId, Long locationId) {
		...
		Map<Long, String> prCurrencyMap = prRepository.findPrIdAndNumberMap(subsidiaryId, locationId, false, status);
		List<IdNameResponse> idNumbers = new ArrayList<IdNameResponse>();
		for (Map.Entry<Long, String> entry : prCurrencyMap.entrySet()) {
			idNumbers.add(new IdNameResponse(entry.getKey(), entry.getValue()));
		}
		return idNumbers;
	}
	...
	public Boolean sendForApproval(Long id) {
		...
		try {
			...
			componentUtility.sendEmailByApproverId(approverIdStr, FormNames.PR.getFormName());
...
//		}
	...
	public Boolean approveAllPrs(List<Long> prIds) {
		...
		try {
			for (Long prId : prIds) {
				...
				componentUtility.sendEmailByApproverId(purchaseRequisition.get().getNextApprover(), FormNames.PR.getFormName());
	...
	private boolean findIsApprovalRoutingActive(Long subsidiaryId) {
		return this.componentUtility.findIsApprovalRoutingActive(subsidiaryId, FormNames.PR.getFormName());
	}

	@Override
	public List<PrItem> findUnprocessedItemsByPrIds(List<Long> prIds) {
		List<PrItem> items = new ArrayList<PrItem>();
		for (Long prId : prIds) {
			items.addAll(this.findUnprocessedItemsByPrId(prId));
		}
		return items;
	}
	...
	public List<IdNameResponse> findApprovedPrsBySubsidiary(Long subsidiaryId) {
		return this.prRepository.findDistinctApprovedPrNumbersBySubsidiary(subsidiaryId, false);
	}
			...
			while (rows.hasNext()) {
				...
				try {
					if (inputCurrentRow.getCell(1) != null) {
						...
						if (subsidiaryId == null) {
							...
							purchaseRequisition.setCurrency(this.subsidiaryRepository.findCurrencyByNameAndIsDeleted(subsidiaryName, false));
						}
					} else {
				...
				try {
					if (inputCurrentRow.getCell(4) != null) {
						...
						Long count = projectRepository.getCountByName(projectName);
				...
				try {
					if (inputCurrentRow.getCell(6) != null) {
						...
						List<Location> locations = this.locationRepository.findByLocationNameAndIsDeleted(locationName, false);
				...
				try {
					if (inputCurrentRow.getCell(10) != null) {
						...
						if (item.isPresent()) {
							prItem.setItemId(item.get().getId());
						} else {
				...
				try {
					if (inputCurrentRow.getCell(11) != null) {
						...
						prItem.setQuantity(quantity.intValue());
					} else {
