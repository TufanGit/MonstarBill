...
import com.monster.bill.dao.EmployeeDao;
...
import com.monster.bill.models.EmployeeContact;
...
import com.monster.bill.repository.EmployeeAccessRepository;
import com.monster.bill.repository.EmployeeAccountingRepository;
import com.monster.bill.repository.EmployeeAddressRepository;
import com.monster.bill.repository.EmployeeContactRepository;
import com.monster.bill.repository.EmployeeHistoryRepository;
import com.monster.bill.repository.EmployeeRepository;
import com.monster.bill.repository.EmployeeRoleRepository;
import com.monster.bill.service.EmployeeService;
	...
	public Employee save(Employee employee) {
		...
		try {
			...
			if (employee.getId() == null) {
				employeeNumber = this.preferencesService.findPreferenceNumberByMaster(employee.getSubsidiaryId(), "Employee Master");
	...
	private void saveEmployeeAccess(EmployeeAccess employeeAccess, Long employeeId, String employeeNumber) {
		...
		if (employeeAccess.isAccess()) {
			...
			if (CollectionUtils.isNotEmpty(employeeAccess.getEmployeeRoles())) {
				...
				userComponentUtility.saveUserCredentials(employeeAccess.getAccessMail(), password, new ArrayList<Long>(roles), isNewRecord);
			}
			log.info("user with roles is saved.");
		} else {
			//delete from user credentials
			if (!StringUtils.isEmpty(employeeAccess.getAccessMail())) {
				userComponentUtility.deleteUserCredentials(employeeAccess.getAccessMail());				
			}
...
//		}
	...
	public Long getEmployeeIdByAccessMail(String email) {
		...
		else { throw new BadCredentialsException("User is not exist in emplyoee."); }
	}
}
