...
import com.monster.bill.common.CustomBadRequestException;
...
import com.monster.bill.dao.PurchaseOrderDao;
import com.monster.bill.enums.FormNames;
import com.monster.bill.enums.MatchType;
import com.monster.bill.enums.Operation;
import com.monster.bill.enums.PaymentTerm;
import com.monster.bill.enums.PoType;
...
import com.monster.bill.payload.request.RfqPoRequest;
import com.monster.bill.payload.response.PaginationResponse;
import com.monster.bill.repository.AccountRepository;
import com.monster.bill.repository.ItemRepository;
import com.monster.bill.repository.PrItemRepository;
import com.monster.bill.repository.PurchaseOrderHistoryRepository;
import com.monster.bill.repository.PurchaseOrderItemRepository;
import com.monster.bill.repository.PurchaseOrderRepository;
import com.monster.bill.repository.PurchaseRequisitionRepository;
import com.monster.bill.repository.QuotationAnalysisItemRepository;
import com.monster.bill.repository.QuotationAnalysisRepository;
...
import com.monster.bill.repository.TaxGroupRepository;
import com.monster.bill.service.ApprovalPreferenceService;
import com.monster.bill.service.DocumentSequenceService;
import com.monster.bill.service.PurchaseOrderService;
import com.monster.bill.service.PurchaseRequisitionService;
import com.monster.bill.service.QuotationAnalysisService;
import com.monster.bill.service.QuotationService;
...
public class PurchaseOrderServiceImpl implements PurchaseOrderService {
	...
	public PurchaseOrder save(PurchaseOrder purchaseOrder) {
		...
		if (purchaseOrder.getId() == null) {
			...
			String documentSequenceNumber = this.documentSequenceService.getDocumentSequenceNumbers(transactionalDate, purchaseOrder.getSubsidiaryId(), FormNames.PO.getFormName(), false);
		...
		if (CollectionUtils.isNotEmpty(poItems)) {
			for (PurchaseOrderItem purchaseOrderItem : poItems) {
				...
				if (PoType.PR_BASED.getPoType().equalsIgnoreCase(purchaseOrder.getPoType())) {
					...
					List<PrItem> prItems = this.prItemRepository.findByPrIdAndItemId(purchaseOrderItem.getPrId(), purchaseOrderItem.getItemId());
					if (CollectionUtils.isNotEmpty(prItems)) {
						for (PrItem prItem : prItems) {
							prItem.setPoId(purchaseOrderSaved.getId());
	...
	public PurchaseOrder findByPoId(Long id) {
		...
		if (CollectionUtils.isNotEmpty(poItems)) {
			for (PurchaseOrderItem poItem : poItems) {
				Optional<Account> account = this.accountRepository.findByIdAndIsDeleted(poItem.getAccountId(), false);
				if (account.isPresent()) poItem.setAccountCode(account.get().getCode());
			}
		}
...
//			Map<String, List<RfqPoRequest>> groupedPr = new HashMap<String, List<RfqPoRequest>>();
	...
	private void preparePo(RfqPoRequest rfqPoRequest, String prNumbersPipeSeprated) {
		...
		String documentSequenceNumber = this.documentSequenceService.getDocumentSequenceNumbers(transactionalDate , rfqPoRequest.getSubsidiaryId(), FormNames.PO.getFormName(), false);
		...
		purchaseOrder.setCurrency(rfqPoRequest.getPrCurrency());
		...
		for (int i=0; i < prNumbers.length; i++) {
			...
			List<PrItem> prItems = purchaseRequisitionService.findUnprocessedItemsByPrId(prId);
			...
			if (CollectionUtils.isNotEmpty(prItems)) {
				for (PrItem prItem : prItems) {
					...
					purchaseOrderItem.setAmount(purchaseOrderItem.getQuantity() * 0.1);
		...
		try {
			if (CollectionUtils.isNotEmpty(qaItems)) {
				...
				for (Map.Entry<String, String> entry : groupedPr.entrySet()) {
					...
					String poNumber = this.documentSequenceService.getDocumentSequenceNumbers(transactionalDate, subsidiaryId, FormNames.PO.getFormName(), false);
	...
	public Boolean sendForApproval(Long id) {
		...
		try {
			...
			componentUtility.sendEmailByApproverId(purchaseOrder.get().getNextApprover(), FormNames.PO.getFormName());
	...
	public PurchaseOrderItem getByPoItemId(Long poId, Long itemId) {
		...
		Item item = itemRepository.findByIdAndIsDeleted(purchaseOrderItem.getItemId(), false).get();
...
//						externalId = inputCurrentRow.getCell(0).getStringCellValue();
				...
				try {
					if (inputCurrentRow.getCell(1) != null) {
						...
						Long subsidiaryId = this.subsidiaryRepository.findIdByNameAndIsDeleted(subsidiaryName, false);
				...
				try {
					if (inputCurrentRow.getCell(2) != null) {
						...
						Optional<Supplier> supplier = this.supplierRepository.findByNameAndIsDeleted(vendorName, false);
				...
				try {
					if (inputCurrentRow.getCell(13) != null) {
						...
						if (item.isPresent()) {
							poItem.setItemId(item.get().getId());
						} else {
				...
				try {
					if (inputCurrentRow.getCell(16) != null) {
						...
						Optional<TaxGroup> taxGroup = this.taxGroupRepository.findByNameAndIsDeleted(taxGroupName, false);
						if (taxGroup.isPresent()) {
							poItem.setTaxGroupId(taxGroup.get().getId());
						} else {
				...
				try {
					if (inputCurrentRow.getCell(19) != null) {
						...
						List<SupplierAddress> addresses = this.supplierAddressRepository.findBySupplierIdAndAddressCodeAndIsDeleted(supplierId, billingAddressCode, false);
				...
				try {
					if (inputCurrentRow.getCell(20) != null) {
						...
						List<SupplierAddress> addresses = this.supplierAddressRepository.findBySupplierIdAndAddressCodeAndIsDeleted(supplierId, shippingAddressCode, false);
						if (CollectionUtils.isEmpty(addresses)) {
