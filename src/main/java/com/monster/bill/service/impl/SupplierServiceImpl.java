...
import com.monster.bill.common.UserComponentUtility;
import com.monster.bill.dao.SupplierDao;
import com.monster.bill.enums.AccountTypes;
...
import com.monster.bill.repository.CustomRoleRepository;
import com.monster.bill.repository.SubsidiaryRepository;
import com.monster.bill.repository.SupplierAccessRepository;
import com.monster.bill.repository.SupplierAccountingRepository;
import com.monster.bill.repository.SupplierAddressRepository;
import com.monster.bill.repository.SupplierContactRepository;
import com.monster.bill.repository.SupplierHistoryRepository;
import com.monster.bill.repository.SupplierRepository;
import com.monster.bill.repository.SupplierRoleRepository;
import com.monster.bill.repository.SupplierSubsidiaryRepository;
import com.monster.bill.service.ApprovalPreferenceService;
import com.monster.bill.service.PreferencesService;
	...
	public Supplier save(Supplier supplier) {
		...
		try {
			...
			if (supplier.getId() == null) {
				...
				String preferenceNumber = this.preferencesService.findPreferenceNumberByMaster(subsidiaryId, "Supplier Master");
	...
	private void saveSupplierAccess(SupplierAccess supplierAccess, Long supplierId) {
		...
		if (supplierAccess.isAccess()) {
			...
			if (CollectionUtils.isNotEmpty(supplierAccess.getSupplierRoles())) {
				...
				userComponentUtility.saveUserCredentials(supplierAccess.getAccessMail(), password,
						new ArrayList<Long>(roles), isNewRecord);				
			}
			log.info("user with roles is saved.");
		} else {
			if (!StringUtils.isEmpty(supplierAccess.getAccessMail())) {
				userComponentUtility.deleteUserCredentials(supplierAccess.getAccessMail());
			}
...
//						throw new CustomException("External ID shoudl not empty.");
				...
				try {
					if (inputCurrentRow.getCell(15) != null) {
						...
						// currency
						supplierSubsidiary.setCurrency(this.subsidiaryRepository.findByIdAndIsDeleted(subsidiaryId, false).get().getCurrency());
					} else {
