...
import com.monster.bill.dao.ItemDao;
...
//						item.setExternalId(inputCurrentRow.getCell(0).getStringCellValue());
				...
				try {
					if (inputCurrentRow.getCell(1) != null) {
						...
						Long subsidiaryId = subsidiaryRepository.findIdByNameAndIsDeleted(subsidiaryName, false);
	...
	public Map<String, Long> getdashboard(Long subsidiaryId) {
		...
		try {
			Long count = itemRepository.countBySubsidiaryIdAndNatureOfItem(subsidiaryId, "Non-Stockable");
			Long count2 = itemRepository.countBySubsidiaryIdAndNatureOfItem(subsidiaryId, "Stockable");
			dashboard.put("Non-Stockable", count);
			dashboard.put("Stockable", count2);
		} catch (Exception ex) {
			ex.printStackTrace();
