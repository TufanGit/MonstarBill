...
import com.monster.bill.dao.QuotationAnalysisDao;
...
import com.monster.bill.repository.QuotationAnalysisHistoryRepository;
...
//	private QuotationService quotationService;
	...
	public QuotationAnalysis save(QuotationAnalysis quotationAnalysis) {
		...
		if (quotationAnalysis.getId() == null) {
			...
			String documentSequenceNumber = this.documentSequenceService.getDocumentSequenceNumbers(transactionalDate, quotationAnalysis.getSubsidiaryId(), FormNames.QA.getFormName(), false);
...
//		}
	...
	public QuotationAnalysis findById(Long id) {
		...
		for (QuotationAnalysisItem quotationAnalysisItem : qaItems) {
			Optional<Location> location = locationRepository.findByIdAndIsDeleted(quotationAnalysisItem.getPrLocationId(), false);
			if (location.isPresent()) {
				quotationAnalysisItem.setPrLocation(location.get().getLocationName());
			}
	...
	public List<Supplier> findSupplierByQaId(Long qaId) {
		...
		if (CollectionUtils.isNotEmpty(suppliers)) {
			supplierMapping = this.supplierRepository.getSupplierNamesByIds(suppliers, false);
		}
	...
	public List<Location> findLocationsByQaIdAndSupplier(Long qaId, Long supplierId) {
		...
		return this.locationRepository.getLocationsByIds(locations);
	}
