...
public interface QuotationItemRepository extends JpaRepository<QuotationItem, String> {
	...
	@Query(" SELECT new com.monster.bill.models.QuotationItem(qi.id, qi.quotationId, qi.rfqNumber, qi.itemId, i.name, i.description, i.uom, qi.quantity, qi.receivedDate, qi.remarks, "
	...
	@Query(" select new com.monster.bill.models.QuotationItem(qi.rfqNumber, qi.quotationId, qi.itemId, i.name, qi.quantity, i.uom, qi.prNumber, qi.prLocation, l.locationName, qi.prId) "
			...
			+ " LEFT JOIN Location l ON l.id = qi.prLocation "
			+ " WHERE qim.vendorId = :vendorId AND qim.quotationId = :quotationId AND qim.isDeleted = :isDeleted ")
