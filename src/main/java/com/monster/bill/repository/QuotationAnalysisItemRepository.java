...
public interface QuotationAnalysisItemRepository extends JpaRepository<QuotationAnalysisItem, String> {
	...
	@Query(" SELECT distinct approvedSupplier FROM QuotationAnalysisItem WHERE qaId = :qaId ")
	...
	@Query(" select new com.monster.bill.models.QuotationAnalysisItem(qai.id, qai.qaNumber, qai.itemId, i.name, i.description, i.integratedId,  " + 
			" case " + 
			"	when lower(i.category) = lower('Inventory Item') " + 
			"	then i.assetAccountId   " + 
			"	else i.expenseAccountId  " + 
			" end as accountCode,  " + 
			" qai.uom, qai.quantity, qai.ratePerUnit, qai.actualRate, qai.recievedDate) " + 
			" FROM QuotationAnalysisItem qai " + 
			" INNER JOIN Item i ON qai.itemId = i.id and i.isActive is true " + 
			" WHERE qai.qaId = :qaId AND qai.approvedSupplier = :supplierId AND qai.prLocationId = :locationId ")
	public List<QuotationAnalysisItem> findItemsByQaAndSupplierAndLocation(Long qaId, Long supplierId, Long locationId);

	@Query(" select new com.monster.bill.models.QuotationAnalysisItem(qa.id, qa.qaNumber, sum(qai.quantity)) from QuotationAnalysis qa "
