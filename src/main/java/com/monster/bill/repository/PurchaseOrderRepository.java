...
public interface PurchaseOrderRepository extends JpaRepository<PurchaseOrder, String> {
	...
	@Query("select new com.monster.bill.models.PurchaseOrder(p.id, p.poNumber, p.subsidiaryId, p.locationId, p.location, p.supplierId, p.poDate, p.rejectedComments, p.poStatus, p.memo, s.name as subsidiaryName, "
	...
	@Query("select new com.monster.bill.models.PurchaseOrder(po.id,po.poNumber,po.poStatus, po.poDate) from PurchaseOrder po where (po.poStatus in :poStatus) AND po.locationId = :locationId AND po.subsidiaryId = :subsidiaryId AND po.isDeleted = :isDeleted AND po.matchType = '3 Way' ")
	...
	@Query("select new com.monster.bill.models.PurchaseOrder(po.id,po.poNumber, po.supplierId, po.currency, su.name as supplierName) from PurchaseOrder po inner join Supplier su ON po.supplierId = su.id where po.id = :poId AND po.isDeleted = :isDeleted")
	...
	@Query(value = "select count(*) from mbl.purchase_order_aud where rev = :poId ", nativeQuery=true)
