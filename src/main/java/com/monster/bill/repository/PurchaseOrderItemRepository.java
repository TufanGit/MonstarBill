...
public interface PurchaseOrderItemRepository extends JpaRepository<PurchaseOrderItem, String> {
	...
	@Query(" select "
			+ "new com.monster.bill.models.PurchaseOrderItem(poi.id, poi.poNumber, poi.poId, poi.itemId, poi.quantity, poi.rate, "
			+ " case  "
			...
			+ " i.uom, i.integratedId, pritem.prNumber) "
	...
	@Query("select new com.monster.bill.models.PurchaseOrderItem(poi.id, poi.poNumber, poi.taxGroupId, poi.poId, poi.itemId,poi.remainQuantity, poi.quantity, poi.rate, i.name as itemName, poi.itemDescription as itemDescription, i.uom as itemUom) "
	...
	@Query("SELECT SUM(amount) FROM PurchaseOrderItem WHERE poId = :poId ")
	...
	public PurchaseOrderItem findByPoIdAndItemIdAndIsDeleted(Long poId, Long itemId, boolean isDeleted);
	...
	@Query(" select new com.monster.bill.models.PurchaseOrderItem(po.id, po.poNumber, po.qaId, poi.quantity) " + 
			" FROM PurchaseOrder po " + 
