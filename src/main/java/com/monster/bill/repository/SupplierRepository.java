...
public interface SupplierRepository extends JpaRepository<Supplier, String> {
	...
	@Query(" select new com.monster.bill.models.Supplier(id, name) from Supplier WHERE isDeleted = :isDeleted ORDER BY name asc ")
	public List<Supplier> findAllSuppliers(@Param("isDeleted") boolean isDeleted);

	@Query(" SELECT new com.monster.bill.models.Supplier(s.id, s.name) FROM Supplier s "
	...
	@Query("select new com.monster.bill.models.Supplier(s.id, s.name, ss.currency as currency) from Supplier s inner join SupplierSubsidiary ss ON s.id = ss.supplierId WHERE s.id = :supplierId and s.isDeleted = :isDeleted ")
	public List<Supplier> getAllCurrencyBySupplier(@Param("supplierId") Long supplierId, @Param("isDeleted") boolean isDeleted);
	
	@Query(" SELECT new com.monster.bill.models.Supplier(s.id, s.name, s.approvalStatus, s.vendorNumber, s.vendorType, s.uin, s.rejectComments, sa.access as supplierAccess, ss.subsidiaryId as subsidiaryId, su.name as subsidiaryName, s.approvedBy, "
	...
	@Query("select new com.monster.bill.models.Supplier(s.id, s.name) from Supplier s WHERE s.id IN :suppliers and s.isDeleted = :isDeleted ")
	...
	public Optional<Supplier> findByIntegratedIdAndIsDeleted(String integratedId, boolean isDeleted);
}
