...
public interface TaxRateRuleRepository  extends JpaRepository<TaxRateRule, String> {	
	...
	@Query("select new com.monster.bill.models.TaxRateRule(tr.id, tr.taxGroupId, tr.taxRateId, t.taxName as taxRateName, t.taxType as taxRateType, t.taxRates as taxRate) from TaxRateRule tr INNER JOIN TaxRate t ON t.id = tr.taxRateId WHERE tr.isDeleted = :isDeleted AND tr.taxGroupId = :taxGroupId ")
