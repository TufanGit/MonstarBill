...
public interface CustomRoleRepository extends JpaRepository<CustomRoles, String> {
	...
	@Query("select new com.monster.bill.models.CustomRoles(cr.id, cr.name, cr.selectedAccess, cr.isActive) from CustomRoles cr where cr.selectedAccess = :selectedAccess AND cr.subsidiaryId = :subsidiaryId and cr.isDeleted = :isDeleted ")
	...
	@Query("select new com.monster.bill.models.CustomRoles(cr.id, cr.name, cr.selectedAccess, cr.isActive) from CustomRoles cr where cr.subsidiaryId = :subsidiaryId and cr.isDeleted = :isDeleted ")
