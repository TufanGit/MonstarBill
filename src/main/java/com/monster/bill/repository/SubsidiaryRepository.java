...
public interface SubsidiaryRepository extends JpaRepository<Subsidiary, String> {
	...
	@Query("select new com.monster.bill.models.Subsidiary(id, name, parentCompany, currency, createdDate) from Subsidiary Where isDeleted = :isDeleted order by name asc ")
	public List<Subsidiary> getAllWithFieldsAndDeleted(@Param("isDeleted") boolean isDeleted);
	
	@Query("select new com.monster.bill.models.Subsidiary(id, name, parentCompany, currency, createdDate) from Subsidiary "
	...
	public Long findIdByNameAndIsDeleted(String subsidiaryName, boolean isDeleted);
	...
	public Optional<Subsidiary> findByIntegratedIdAndIsDeleted(String integratedId, boolean isDeleted);
}
